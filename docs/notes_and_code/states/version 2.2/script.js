...
var ROADLEVELS = {
  ...
  trunk_link: 1,
  unclassified: 1
}; //used to compare importance of roads
...
isIntersection:((k===0)?true:false)}); //adds each point to pts var
            }
          }
        }
  ...
  function elevation(){
    ...
    function MakeRequest(points,delay,range){
      expected_requests++;
      setTimeout(function(){
        elevationService.getElevationForLocations({locations:points},function(results,status){
          if(status===google.maps.ElevationStatus.OK){
            for(var i=0; i<results.length; i++){
              pts[i+range.min].elevation = results[i].elevation;
            }
            completion_counter++;
          }else{
            throw("Problem with Elevation Service at Range: "+range.min+" to "+range.max+". Error: "+status);
          }
        });
      },delay);
    }
    ...
    for(var i=0; i<Math.ceil(pts.length/300)*300; i+=300){
      var length = Math.min(300,pts.length-i);
      var request_pts = pts.slice(i,i+length);
      request_pts = request_pts.map(function(a){return a.pt});
      MakeRequest(request_pts,i/300*2000,{min:i, max:i+length});
    }
  ...
  function add_signals(){//Integrate OpenStreetMap Data into pts
    ...
    for(var i=0; i<signals.length; i++){
      siglist.push({lat:parseFloat(signals[i].attributes.lat.value), lng:parseFloat(signals[i].attributes.lon.value)});
    }
    ...
    /*for(var i=0; i<pts.length; i++){
      ...
      for(var j=0; j<siglist.length; j++){
        if(Math.abs(siglist[j].lat-pts[i].pt.lat())<0.001 && Math.abs(siglist[j].lng-pts[i].pt.lng())<0.001){
          pts[i].stop = "light";
          pts[i].speed = 0;
          try{
            pts[i-1].stop = "light";
      ...
      if(!matched && eind.indexOf(pts[i].index)!==-1){
        pts[i].stop = "sign";
        pts[i].speed = 0;
        try{
          pts[i-1].stop = "sign";
        ...
        //will need to replace with smart sign detection
      }
    }*/
    ...
    for(var i=0; i<siglist.length; i++){
      ...
      for(var j=0; j<eind.length; j++){
        if(Math.abs(siglist[i].lat-pts[eind[j]].pt.lat())<0.001 && Math.abs(siglist[i].lng-pts[eind[j]].pt.lng())<0.001){
          pts[eind[j]].stop = "light";
          pts[eind[j]].speed = 0;
          try{
            pts[eind[j]-1].stop = "light";
            pts[eind[j]-1].speed = 0;
          }catch(err){};
          matched = true;
        }
      }
      if(!matched){
        for(var j=0; j<pts.length; j++){
          if(Math.abs(siglist[i].lat-pts[j].pt.lat())<0.0002 && Math.abs(siglist[i].lng-pts[j].pt.lng())<0.0002){
            pts[j].stop = "light";
            pts[j].speed = 0;
          }
        }
    ...
    for(var i=0; i<pts.length-1; i++){ //trim off extra lights in a series
      if(pts[i].stop === "light" && pts[i+1].stop === "light" && !pts.isIntersection){
        pts[i].stop = null;
      }
    }
    }
...
//energy to move along section of road in Joules (n*m)
      ...
      for(var i=0; i<this.length; i++){
        ...
        for(var Pi=0; Pi<this[i].indexes.length-1; Pi++){
          ...
          if(conind.indexOf(this[i].indexes[Pi+1])!==-1){
            ...
            this.path = this.path.concat(intersections.info[this[i].indexes[Pi]].connections[conind.indexOf(this[i].indexes[Pi+1])].path);
          }
        }
        for(var i=1; i<path.length; i++){
          if(this.path[i].stop==="sign"){
            var speed1 = this.path[i-1].road_data.speed;
            var speed2 = this.path[i+1].road_data.speed;
          }
        }
        this[i].cost = total;
      }
    ...
    for(var i=1; i<optimal.path.length-1; i++){
      if(optimal.path[i-1].road_name.real!==optimal.path[i].road_name.real || optimal.path[i].stop==="light"){
