/*VERSION 1.6
summary: added collection of road data, including speed limit, elevation and distance. 
Compiled this data into individual points.
Calculated energy needed to move over an edge of the intersections network.
8:07 AM 2/5/2015
*/
  ...
  function getDirs(req){
    setTimeout(
      directionsService.route(dir_requests[req], function(response, status){
        directionsDisplay.setDirections(response);
        var tb = response.routes[0].bounds; //temporary bounds
        bounds = {north:tb.getNorthEast().lat(), east:tb.getNorthEast().lng(), south:tb.getSouthWest().lat(), west:tb.getSouthWest().lng()};
...
last: {dist: null, slope: null, index: null}}); //adds each point to pts var
      ...
      for(var j=0; j<d.length; j++){
        ...
        for(var r=0; r<roads.length; r++){
          if(!(roads[r] in output)){
            output[roads[r]] = {data:[], speed:null};
          }
  ...
  function add_signals(){//Integrate OpenStreetMap Data into pts
    ...
    function average(d,type){ //returns median speed limit
      var list = [];
    ...
    for(var i=0; i<over.length; i++){
      ...
      for(var j=0; j<pts.length; j++){
        ...
        if(b.north>lat && b.south<lat && b.west<lng && b.east>lng){
          ...
          if(rn.real in info){
            pts[j].road_data = info[rn.real];
          }else if(rn.reversed in info){
            pts[j].road_data = info[rn.reversed];
          }else if(rn.plain in info){
            pts[j].road_data = info[rn.plain];
          }

          //add speed limit
          if(pts[j].road_data!==null){
            if(pts[j].road_data.data.some(function(a){return (a.maxspeed)?true:false})){ //has expicit speed
              var sum = 0; var count = 0;
              pts[j].road_data.data.map(function(a){if(a.maxspeed!==undefined){sum += parseInt(a.maxspeed)*METERPS*a.nodes; count += a.nodes}});
              pts[j].road_data.speed = sum/count;
            }else if(pts[j].road_data.data[0].highway in speeds[pts[j].road_data.data[0]["tiger:county"]]){ //has data from other roads of it's type
              pts[j].road_data.speed = speeds[pts[j].road_data.data[0]["tiger:county"]][pts[j].road_data.data[0].highway].average;
            }else{ //NOTHING
              pts[j].road_data.speed = speeds[pts[j].road_data.data[0]["tiger:county"]].default;
            }
          }else{
            pts[j].road_data = {data:[], speed:35*METERPS};
          }
        }
      }
    }
...
dist += a.next.dist; 
try{speed += a.road_data.speed * a.next.dist; speed_dist += a.next.dist}catch(err){}/*in case road_data is undefined*/});
...
dist += a.last.dist;
try{speed += a.road_data.speed * a.last.dist; speed_dist += a.last.dist}catch(err){}});
