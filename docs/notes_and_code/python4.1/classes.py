from ecocartographer.polyline import PolylineCodec
from ecocartographer import util
import math
...
class Instruction(object):
    def __init__(self, path, command, feature, vehicle):
        self.path = path
        '''self.polyline = Polyline.encode('|'.join(map(lambda a: a.string, path)))
        self.distance = sum(map(lambda a: a.pt.distanceTo(a.next.pt), path[:-1]))
        self.energy = sum(map(lambda a: a.getEnergyNext(vehicle), path[:-1]))'''
...
class Path(list):
    ...
    def getInstructions(self, vehicle):
        ...
        for pt in self[slice_start:-2]:
            ...
            if pt.roadname != pt.next.roadname or pt.next.isStoplight:
                feature = 'turn'
                if self[slice_start].temp_speed == 0: feature = 'stopsign'
...
class Pt(object):
    ...
    def __init__(self, latLng, index, speed, roadname, isIntersection=False):
        ...
        self.pt = latLng
        self.index = index
        self.isInterection = isIntersection
        self.string = latLng.toStringURL()
        self.isStoplight = False
        self.isStopsign = isIntersection
