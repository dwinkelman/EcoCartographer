import classes, util, query, polyline, ecio
...
def GetPoints(start, end, console):
    ...
    console.add('Set Directions Data', error=': '+str(t1-t0))
...
def References(pts, console):
    ...
    console.add('References', error=': '+str(time.time()-t0))
...
def IntersectionsJoin(pts, indexes, console):
    ...
    console.add('Intersections Join', error=': '+str(time.time()-t0))
...
def IntersectionsTrim(inter, console):
    ...
    console.add('Intersections Trim', error=': '+str(time.time()-t0))
...
def IntersectionsBuild(inter, pts, console):
    ...
    console.add('Build Intersections', error=': '+str(time.time()-t0))
...
def OptimalDistance(intersections, console, factor=2):
    ...
    Branch([], start, 0)
    t1 = time.time()
    console.add('Recursive', error=': '+str(t1-t0))
    return solutions
...
def OptimalEnergyDetailed(intersections, routes, vehicle, number, console):
    ...
    new_routes.sort(key=lambda a: a.energy)

    console.add('Detailed Energy', error=': '+str(time.time()-t0))
        
    return new_routes[:number]
...
def EcoCartographer(args):
    ...
    routes_force = OptimalEnergyInitial(final_inter, routes, args['routes'], Console)
    recalc = OptimalEnergyDetailed(final_inter, routes_force, vehicle, args['routes'], Console)
...
if len(sys.argv) < 2:
    ...
    #filter intersections by being in calculated intersections already
    '''intsInRange = util.uniquify(util.flatten(map(lambda a: a[0], routes))) #makes list of intersections within reasonable distance to start/end'''
    keys = list(set([inter for route, dist in routes for inter in route]))
    '''ultimate_trim = sorted(util.flatten(map(lambda a: intersections[a].references[0].references, intsInRange))) #points included in intersections'''
    ultimate_trim = sorted([index for inter in keys for index in intersections[inter].references[0].references])
    intsInRange = IntersectionsBuild(IntersectionsJoin(pts, ultimate_trim, Console), pts, Console)
    '''final_refs = ValidReferences(final_inter)'''

    #get elevation data of relevant pts
    '''Add in support for interpolation'''
    elev_refs, interpol_refs = Elevator.getInterpolations(routes, intsInRange, Console) #
    final_refs = elev_refs #
    final_refs.extend([ref for ref, ref1, ref2 in interpol_refs]) #
    ref_chunks = Overpasser.chunk(pts, final_refs)
    pts = Overpasser.overpass(pts, final_refs, ref_chunks, bounds, Console)
    pts, elev_queries, success = Elevator.elevation(pts, elev_refs, interpol_refs, Console)
    ...
    routes_force = OptimalEnergyInitial(intsInRange, routes, args['routes'], Console) #
    recalc = OptimalEnergyDetailed(intsInRange, routes_force, vehicle, args['routes'], Console) #
    recom = Path(recom, vehicle)
