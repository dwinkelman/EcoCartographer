...
class PolylineCodec(object):
    ...
    def encode(self, a):
        '''Encode list of LatLng tuples/strings to Polyline format.'''
        output = six.StringIO()
        coordinates = None
        ...
        if type(a) == list or type(a) == tuple:
            if type(a[0]) == str:
                coordinates = '|'.join(a)
            if type(a[0]) == classes.LatLng:
                coordinates = [(pt.lat,pt.lng) for pt in a]
        elif type(a) == str:
            coordinates = []
            for i in a.split('|'):
                s = i.split(',')
                coordinates.append((float(s[0]),float(s[1])))
