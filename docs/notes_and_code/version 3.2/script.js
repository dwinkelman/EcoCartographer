...
//variables to go inside Network()
  ...
  var has_elevation; //array with numbers to determine status of elevation/interpolation
          ...
          for(var j=0; j<L.steps.length; j++){
            ...
            for(var k=0; k<pc.path.length; k++){
              pts.push({
                ...
                isIntersection:false, //set to true if it is an initial, NOT final, intersection (see intersection_gen())
                isInIntersection:false, //set to true if it is in the final network, used to compile elevation data
...
success:Callback
  ...
  function elevation(){ //Get elevation data from Google Maps Elevation Service and assign that (and other data) to pts
    ...
    function MakeRequest(data,delay){
      var points = data.map(function(a){return a[0]});
      expected_requests++;
      setTimeout(function(){
          ...
          if(status===google.maps.ElevationStatus.OK){
            for(var i=0; i<results.length; i++){
              for(var j=0; j<data[i][1].length; j++){
                pts[data[i][1][j]].elevation = results[i].elevation;
              }
            }
            completion_counter++;
            elev_value.innerText = parseInt(Math.round(completion_counter/expected_requests*100))+"%";
    ...
    function BottleRequest(data, delay){
      var waiter = setTimeout(function(){
        $.ajax({
          method:"GET",
          url:"http://localhost:8080/get-elevation/"+data.string,
          dataType:'json',
          success:function(a){ElevationCallback(a,data)},
          error:function(){ElevationError(data)}
        });
      },delay);
    }
    function ElevationCallback(elevation,points){
      if(elevation.status === "OK"){
        for(var i=0; i<elevation.results.length; i++){
          var refs = points[i];
          for(var j=0; j<refs.length; j++){
            pts[refs[j]].elevation = elevation.results[i].elevation;
          }
        }
        completion_counter++;
        elev_value.innerText = parseInt(Math.round(completion_counter/expected_requests*100))+"%";
      }else{
        ElevationError(points);
      }
    }
    function ElevationError(data){
      failed_counter++;
      failed_requests.push(data);
      console.log("Failed Elevation Request.");
    }

    //Elevation
    var e = pts.map(function(a){return a.isInIntersection});
    var requests = [];
    var encode = google.maps.geometry.encoding.encodePath;
    ...
    while(i<e.length){
      ...
      while(nri<188){
        if(e[i]){
          nri++;
          new_req.push(pts[i].references);
          coords.push(pts[i].pt);
          for(var j=0; j<pts[i].references.length; j++){
            e[pts[i].references[j]] = false;
          }
    ...
    var expected_requests = requests.length;
    for(var i=0; i<requests.length; i++){
      BottleRequest(requests[i],i*100);
    }
      ...
      if(completion_counter + failed_counter === expected_requests){
        if(completion_counter === expected_requests){
          ...
          for(var i=0; i<failed_requests.length; i++){
            BottleRequest(failed_requests[i],i*100);
          }
          failed_requests = [];
        }
      }
  ...
  function intersection_build(){ //Get data for intersections
    ...
    for(var i=0; i<series.length-1; i++){
      if(series[i].int_index!==series[i+1].int_index){
        if(!(series[i+1].int_index in intersections[series[i].int_index].connections)){
          for(var j=series[i].pt_index; j<series[i+1].pt_index+1; j++){
            pts[j].isInIntersection = true;
          }
  ...
  function intersection_build_part2(){
