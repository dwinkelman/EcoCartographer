    ...
    def getInterpolations(self, routes, intersections, console):
        ...
        for route, dist in routes:
            for int1, int2 in zip(route[:-1], route[1:]):
                if not ((int2, int1)) in pairs:
                    pairs.add((int1, int2))
                cons_used.add((int1, int2))

        for index, con in list(pairs):
            path = intersections[index][con].path
            path_len = len(path)
        ...
        console.add('Interpolations', error=': '+str(round(time.time()-t0,3)))
        return list(elevation_out), list(interpolate_out), list(cons_used)
        ...
        for ref in references:
            for index in ref:
                if pts[index].isStopsign:
                    ...
                    break
        t3 = time.time()
        console.add('Overpass (query)', error=': '+str(round(t1-t0,3)))
        console.add('Overpass (add to points)', error=': '+str(round(t2-t1,3)))
        console.add('Overpass (add to references)', error=': '+str(round(t3-t2,3)))
        return pts
